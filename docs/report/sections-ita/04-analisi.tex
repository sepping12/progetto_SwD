...
\begin{itemize}
    ...
    \item \texttt{mutation-testing.yml}: Media 18 secondi (161 test + 21 mutanti)
...
\textbf{Mutation Score Complessivo}: \textbf{100\%} (21/21 mutanti uccisi)
...
Test Strength & 100\% \\
Numero Test Eseguiti & 161 \\
Test per Mutazione & 5.62 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{Operatori di Mutazione PITest con Mutatori STRONGER}
\label{tab:mutation-operators}
\begin{tabular}{llr}
\toprule
\textbf{Operatore} & \textbf{Descrizione} & \textbf{Count} \\
\midrule
VoidMethodCallMutator & Rimuove chiamate metodi void & 8/8 \\
RemoveConditionalMutator\_EQUAL\_ELSE & Testa branch else & 5/5 \\
RemoveConditionalMutator\_EQUAL\_IF & Testa branch if & 5/5 \\
NullReturnValsMutator & Verifica gestione null & 2/2 \\
EmptyObjectReturnValsMutator & Testa oggetti vuoti & 1/1 \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{Strategia di Esclusione Codice Infrastruttura}

\textbf{Problema}: Il codice di configurazione Spring produceva mutanti difficili da testare senza integration tests complessi.

\textbf{Soluzione}: Configurazione PITest per escludere config package e focus su business logic:

\begin{lstlisting}[language=XML, caption=Configurazione PITest con Mutatori STRONGER]
<plugin>
...
\textbf{Impatto}: 28 mutanti (68\% killed) $\rightarrow$ 21 mutanti (100\% killed), focus su logica business con mutatori più rigorosi.
