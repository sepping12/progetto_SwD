...
Sono stati configurati quattro workflow GitHub Actions specializzati:
...
\texttt{docker-build.yml} & Push main & Build e push immagine Docker \\
\texttt{maven-schedule.yml} & Daily 00:00 UTC & Controlli qualità schedulati \\
\texttt{mutation-testing.yml} & Manual/Weekly & Analisi mutation PITest \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Performance}:
\begin{itemize}
    \item \texttt{maven.yml}: Media 45 secondi (build, test, coverage, analisi)
    \item \texttt{docker-build.yml}: Media 2 minuti (multi-stage build + push)
...
\begin{itemize}
    ...
    \item Assicurazione qualità giornaliera
\end{itemize}
