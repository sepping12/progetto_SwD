...
\textbf{Strategia}: Separazione concerns in sei workflow specializzati con orchestrazione
...
\midrule
\texttt{ci.yml} & Ogni push/PR & Build, test, coverage rapidi \\
\texttt{build.yml} & Push main & Creazione immagine produzione \\
\texttt{security.yml} & Push/PR/Weekly & 7 tool security scanning \\
\texttt{mutation-testing.yml} & Push/Manual & Analisi qualità test \\
\texttt{performance-regression.yml} & Weekly/Manual & Monitoraggio performance \\
\texttt{pipeline-orchestrator.yml} & Push/Manual & Esecuzione sequenziale coordinata \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Impatto}:
\begin{itemize}
    \item Feedback loop CI veloce: 7 minuti (build + coverage + SonarCloud)
    \item Security completo: 16 minuti con 7 tool specializzati
    \item Orchestrazione totale: 40 minuti per analisi completa
...
\textbf{Innovazione}: Workflow centrale che coordina l'esecuzione sequenziale di tutti gli altri workflow.

\textbf{Architettura}:
\begin{enumerate}
    \item Esecuzione CI pipeline (build + test + coverage)
    \item Attesa completamento e build Docker
    \item Security scan completo (7 tool)
    \item Mutation testing con PITest
    \item Performance regression testing con JMH
\end{enumerate}

\textbf{Vantaggi}:
\begin{itemize}
    \item Gestione dipendenze tra workflow
    \item Summary unificato dell'intera analisi
    \item Trigger singolo per esecuzione completa
    \item Facilita debugging con visione d'insieme
\end{itemize}
